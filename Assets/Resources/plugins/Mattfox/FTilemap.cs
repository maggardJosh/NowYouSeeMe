...
public class FTilemap : FContainer
{
			...
			if (line != "") { // skip empty rows
				...
				foreach (string frame in frames) {
					if (frame != "") { 
						...
						_tileArray [i + (j * _tilesWide)] = frameNum-firstGID;
	...
	protected void buildTiles ()
	{
        ...
        int clipTilesWide = Mathf.FloorToInt((_clipWidth / _tileWidth) + 2);
		if (_tilesWide < clipTilesWide || _clipWidth == 0 || _skipZero) {
			clipTilesWide = _tilesWide;
		}
		int clipTilesHigh = Mathf.FloorToInt ((_clipHeight / _tileHeight)+2);
